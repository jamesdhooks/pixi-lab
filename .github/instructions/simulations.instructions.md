...
16. **Manual demo QA rule** — new demo-capable simulations remain visually marked as needing QA in the demo gallery until James explicitly approves them. When approval happens, use `.github/skills/qa-experience/SKILL.md` to update both `pixijs_simulation_tracking_system_v1.md` and `packages/demo/src/demoQaStatus.ts` in the same change.
