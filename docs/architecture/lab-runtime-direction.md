...
Pixi Lab main now carries the reusable browser Lab Runtime plus a deliberately small curated content set: Ball Pit, Haromonics, and Space Debris. The runtime should support games, simulations, previews, overlays, and future renderer backends, but main should only promote cleaned-up content one slice at a time.
