...
11. Preview scenes must be cheap: no audio, no large raw textures, no unbounded physics churn.
12. Keep rendering idle-aware. Do not force continuous render unless there is animation, awake physics, active input, particles/effects, or a documented custom visual reason.
13. `aiAutoplay` means supported, not always running. AI should inject through the normal input/model path.
14. Settings fields need sections. Input-mode-specific controls belong under `section: 'Input Mode'` and should use `visibleModes` where available.
15. Capabilities must match reality. Do not advertise raw/WebGL/gamepad/score/tutorial/demo if the implementation does not support it.
...
| T1 | Pegboard / Pachinko | touch | qa-needed | Drop balls through pegs into scoring bins. | Implemented as `packages/games/src/pegboard/` with deterministic `PegboardModel`, explicit `start` → `play` → `result` state path, shell-owned reset/restart, score/game-over events, style/settings-only game controls, Pixi-first primitive scene, cheap preview, settings, tutorial, and opt-in AI dropper. Visual polish includes neon board glow, centered/staggered colorful pegs, aligned bins, ball trails, score popups, bursts/sparks, jackpot bin emphasis, and result-state visual overlay. | **Updated 2026-07-08:** games no longer advertise engine configurations/quality modes; scene-owned title/explainer/top-row text removed; balls drop directly under input x; pegs/bins derive from one explicit centered board bounds contract; rows are staggered by half-spacing to avoid vertical lanes. Validation includes focused Pegboard tests for game controls, input mapping, bounds alignment, and staggered rows. Manual browser/device QA remains before acceptance. |
