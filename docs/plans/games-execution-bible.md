...
4. Every rule-driven game must expose an explicit state path in the model: `start` → `play` → `result`, with shell-owned `reset` returning to `start`. Do not hide game state solely in scene visuals.
5. Reuse the Pixi Lab shell UI for reusable controls: intro/tutorial pages explain Start/Play/Result/Restart, Reset calls `Scene.reset()`, score flows through `score_update`, and final result flows through `game_over`. Add custom React UI only when the shell cannot express the workflow.
6. Games must never expose an “Engine” / “Engine configuration” dropdown. Do not advertise `qualityModes` or `engineConfigurations` for games; render variants must be style/mode/settings choices only.
7. Game scenes must avoid the shared top-row UI safe area. That area belongs to common shell controls and score. Do not place scene-owned score, title, state labels, explainers, or controls there.
8. Game scenes must not render their own title or explainer text. Put those in `name`, `short`, `long`, and `tutorialPages` so the IntroCard/Info surfaces own explanation.
9. Board/gameplay geometry must share a single explicit bounds contract when alignment matters: pegs, walls, drop zone, sensors, and buckets/bins should derive from the same bounds rather than separate percentages.
10. Touch games must map user input coordinates directly to the gameplay affordance. Do not add hidden random jitter to drop/spawn positions; randomize velocity/effects instead.
11. Pegboards/boards must be responsive: board bounds, peg count, peg radius, row/column spacing, and bucket geometry should adapt to the current viewport instead of using a fixed peg count. Use most of the screen while preserving the shared top-row UI safe area.
12. Scoring games must make the active score and remaining attempts/balls readable in the shared shell UI, not tiny badges.
13. Any scoring bucket/bin/target must visibly label its point value or multiplier in-scene.
14. Preview scenes must be cheap: no audio, no large raw textures, no unbounded physics churn.
15. Keep rendering idle-aware. Do not force continuous render unless there is animation, awake physics, active input, particles/effects, or a documented custom visual reason.
16. `aiAutoplay` means supported, not always running. AI should inject through the normal input/model path.
17. Settings fields need sections. Input-mode-specific controls belong under `section: 'Input Mode'` and should use `visibleModes` where available.
18. Capabilities must match reality. Do not advertise raw/WebGL/gamepad/score/tutorial/demo if the implementation does not support it.
...
| T1 | Pegboard / Pachinko | touch | qa-needed | Drop balls through pegs into scoring bins. | Implemented as `packages/games/src/pegboard/` with deterministic `PegboardModel`, explicit `start` → `play` → `result` state path, shell-owned reset/restart, readable score/balls HUD, score/game-over events, style/settings-only game controls, Pixi-first primitive scene, cheap preview, settings, tutorial, and opt-in AI dropper. Visual polish includes neon board glow, responsive centered/staggered colorful pegs, aligned labeled multiplier bins, ball trails, score popups, bursts/sparks, jackpot bin emphasis, and result-state visual overlay. | **Updated 2026-07-08:** games no longer advertise engine configurations/quality modes; scene-owned title/explainer/top-row text removed; balls drop directly under input x; pegs/bins derive from one explicit centered board bounds contract; rows are staggered by half-spacing to avoid vertical lanes; board footprint/peg density adapt to viewport; each bucket labels its multiplier; shell HUD shows larger Score + Balls remaining. Validation includes focused Pegboard tests for game controls, input mapping, responsive density, bounds alignment, bucket labels, and staggered rows. Manual browser/device QA remains before acceptance. |
| T2 | Color Pit Arena | touch | in-progress | Route colored balls into matching bins before overflow. | Implemented as separate `packages/games/src/color-pit-arena/` with deterministic `ColorPitModel`, explicit `start` → `play` → `result` state path, shell-owned reset/tutorial/settings, color-lane scoring, overflow bust pressure, cheap preview, and opt-in AI nudges. | **Updated 2026-07-09:** selected as the second touch game after Pegboard; preserves Ball Pit simulation behavior and applies Pegboard lessons: no engine dropdowns, settings sections, bounded preview cadence, idle-aware scene rendering, and no scene-owned title/top-row chrome. Validation evidence to be filled after gates complete. |
