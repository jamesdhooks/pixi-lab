...
6. Preview scenes must be cheap: no audio, no large raw textures, no unbounded physics churn.
7. Keep rendering idle-aware. Do not force continuous render unless there is animation, awake physics, active input, particles/effects, or a documented custom visual reason.
8. `aiAutoplay` means supported, not always running. AI should inject through the normal input/model path.
9. Settings fields need sections. Input-mode-specific controls belong under `section: 'Input Mode'` and should use `visibleModes` where available.
10. Capabilities must match reality. Do not advertise raw/WebGL/gamepad/score/tutorial/demo if the implementation does not support it.
...
| T1 | Pegboard / Pachinko | touch | qa-needed | Drop balls through pegs into scoring bins. | Implemented as `packages/games/src/pegboard/` with deterministic `PegboardModel`, explicit `start` → `play` → `result` state path, shell-owned reset/restart, score/game-over events, Pixi-first primitive scene, cheap preview, settings, tutorial, and opt-in AI dropper. Visual polish includes neon board glow, colorful pegs/bins, ball trails, score popups, bursts/sparks, jackpot bin emphasis, and result overlay. | **Implemented 2026-07-08:** files touched include `PegboardModel.ts`, `PegboardScene.ts`, `PegboardPreviewScene.ts`, `PegboardAI.ts`, `pegboard.config.ts`, `pegboard.definition.ts`, tests, and `packages/games/src/index.ts`. Validation: focused Pegboard tests `6 passed`; `pnpm --filter @hooksjam/pixi-lab-core build` passed; `pnpm --filter @hooksjam/pixi-lab-games typecheck` passed; `pnpm --filter @hooksjam/pixi-lab-games build` passed; root `pnpm build` passed; `pnpm --recursive typecheck` passed; root `pnpm test` passed (`29` files / `156` tests); `pnpm lint` passed after fixing an existing core `prefer-const` lint blocker. Manual browser/device QA remains before acceptance. |
