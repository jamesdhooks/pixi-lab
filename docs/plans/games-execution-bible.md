...
| T1 | Pegboard / Pachinko | touch | planned | Drop balls through pegs into scoring bins. | Minimal deterministic `PegboardModel`; Pixi-first primitive renderer; static peg/wall/bin geometry; large touch drop zone; score/bin/game-over events through existing `GameEvent`; reset/tutorial/settings capabilities; cheap preview; opt-in AI dropper. Defer generic physics/effects extraction until Pegboard proves the seam. | **Next vertical slice (2026-07-08):** create `packages/games/src/pegboard/`; write `PegboardModel` tests for deterministic layout, ball drop, bin scoring, reset, and max-ball/game-over guard; add definition/config/tutorial with touch controls (`tap/drag x-position to drop`, Reset via shell); implement primitive Pixi scene with bounded circle bodies and bin sensors; add preview + AI dropper; register in `GAME_REGISTRY`; run focused games tests/build before broad gates. Acceptance: game appears as `kind: 'game'`, emits `score_update` and `game_over` when appropriate, reset clears score/balls, preview is low-budget, no raw/gamepad advertised. |
