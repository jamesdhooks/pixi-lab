...
export class FluidTankDemoAI implements SimulationAI {
  ...
  private doOverhaul(ctx: SimAIContext): void {
    ...
    const [cellSize, fingerForce, fingerRadius, viscosity, curl, eddyAssist, pressureIterations] =
      PARAM_PRESETS[Math.floor(Math.random() * PARAM_PRESETS.length)];
    ctx.applyNumericSetting('cellSize', cellSize);
    ctx.applyNumericSetting('fingerForce', fingerForce);
    ...
    ctx.applyNumericSetting('pressureIterations', pressureIterations);
    ctx.applyNumericSetting('dyePersistence', 0.9992 + Math.random() * 0.0008);
    this.nextOverhaulIn = 20 + Math.random() * 15;
    this.elapsedSinceOverhaul = 0;
  }
}
